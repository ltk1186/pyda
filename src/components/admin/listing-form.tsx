...
type AdminListingFormProps = {
  action: (
    state: AdminListingFormState,
    formData: FormData,
  ) => Promise<AdminListingFormState>;
  creators: AdminListingCreatorOption[];
  listing?: {
    creatorId: string;
    creatorName: string;
    title: string;
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-5">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          <div>
            ...
            {editing ? (
              <>
                <input name="creatorId" type="hidden" value={listing?.creatorId} />
                <p className="mt-2 rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm">
                  {listing?.creatorName}
                </p>
              </>
            ) : (
            ...
            <FieldError message={state.errors?.creatorId} />
          </div>
          <TextField
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-5">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          ...
          <label className="flex items-center gap-3 self-end rounded-md border border-neutral-200 px-3 py-2 text-sm">
            <input
              className="h-4 w-4"
              defaultChecked={listing?.isSample ?? false}
              name="isSample"
              type="checkbox"
            />
            예시 광고 상품
